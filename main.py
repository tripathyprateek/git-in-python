...
def http_request(url, username, password):
    response = urllib.request.Request.get(url, auth=(username, password))
    response.raise_for_status()
    return response.content


def get_remote_master_hash(git_url, username, password):
    """Get commit hash of remote master branch"""
...
def push(git_url, username=None, password=None):
    """Push master branch to given git repo"""
    if username is None:
...
if __name__ == '__main__':
    ...
    sub_parser.add_argument('-a', '--author',
            help='commit author in format "YOUR_NAME <example@example.com>" ')

    sub_parser.add_argument('-m', '--message', required=True,
            help='writes commit message')

    sub_parser = sub_parsers.add_parser('diff',
            help='shows files changed')

    sub_parser = sub_parsers.add_parser('hash-object',
            help='hash contents of given path')
    ...
    sub_parser.add_argument('-w', action='store_true', dest='write',
            help='write object to object store ')

    sub_parser = sub_parsers.add_parser('init',
            help='initialises a new repo')
    ...
    sub_parser.add_argument('-s', '--stage', action='store_true',
            help='show details of object')

    sub_parser = sub_parsers.add_parser('push',
            help='push master branch to given git server')

    sub_parser.add_argument('git_url',
            help='URL of git repo to be pushed ')
    ...
    sub_parser = sub_parsers.add_parser('status',
            help='status of working directory')
